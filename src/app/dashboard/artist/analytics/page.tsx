...
import { BarChart3, TrendingUp, Users, DollarSign, ArrowLeft, Star, Loader2 } from "lucide-react";
...
export default function ArtistAnalytics() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const profileRes = await fetch(`/api/user?action=artist-profile&userId=${user.id}`);
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {stats.map(({ icon: Icon, label, value, change, color, bg }) => (
                <div key={label} className={`p-5 ${bg} rounded-2xl border border-gray-100 dark:border-neutral-800`}>
                  ...
                  <p className="text-xs text-green-500 flex items-center gap-1 mt-1"><TrendingUp className="w-3 h-3" /> {change}</p>
                </div>
              ))}
            </div>
