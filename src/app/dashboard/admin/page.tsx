...
interface Studio { id: string; name: string; email: string; phone: string; location: string; rating: string; createdAt: string; image?: string; bio?: string; }
...
export default function DashboardAdmin() {
  ...
  const [editStudioForm, setEditStudioForm] = useState({ name: "", email: "", phone: "", location: "", image: "", bio: "" });
  ...
  return (
    <>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setEditingArtist(null)}>
                <div className={`bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-neutral-800 w-full max-w-lg ${isMobile ? 'max-w-full mx-0 rounded-none h-full max-h-full' : ''} overflow-y-auto`} onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Profile Picture</label>
                      <ProfilePictureUploader
                        value={editArtistForm.image}
                        onChange={(url) => setEditArtistForm((f) => ({ ...f, image: url }))}
                        folder="profile"
                        size="md"
                      />
                    </div>
            ...
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-neutral-800">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="p-8 text-center text-gray-400">No studios found</td></tr>
                    ...
                    <tr key={studio.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800">
                      ...
                      <td className="p-3">
                        <button
                          ...
                          className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 text-gray-500 dark:text-gray-400 transition-all"
                          title="Edit Studio"
                        >
                          <Settings className="w-4 h-4" />
                        </button>
        ...
        {editingStudio && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setEditingStudio(null)}>
            <div className={`bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-neutral-800 w-full max-w-lg ${isMobile ? 'max-w-full mx-0 rounded-none h-full max-h-full' : ''} overflow-y-auto`} onClick={(e) => e.stopPropagation()}>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Edit Studio</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Profile Picture</label>
                  <ProfilePictureUploader
                    value={editStudioForm.image}
                    onChange={(url) => setEditStudioForm((f) => ({ ...f, image: url }))}
                    folder="profile"
                    size="md"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Name</label>
                    <input type="text" value={editStudioForm.name} onChange={(e) => setEditStudioForm((f) => ({ ...f, name: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-400" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Email</label>
                    <input type="email" value={editStudioForm.email} onChange={(e) => setEditStudioForm((f) => ({ ...f, email: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-400" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Phone</label>
                    <input type="text" value={editStudioForm.phone} onChange={(e) => setEditStudioForm((f) => ({ ...f, phone: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-400" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Location</label>
                    <input type="text" value={editStudioForm.location} onChange={(e) => setEditStudioForm((f) => ({ ...f, location: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-rose-400" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wider">Bio</label>
                  <textarea rows={3} value={editStudioForm.bio} onChange={(e) => setEditStudioForm((f) => ({ ...f, bio: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-rose-400 resize-none" />
                </div>
              </div>
              <div className="flex items-center justify-end gap-2 mt-6">
                <button onClick={() => setEditingStudio(null)} className="px-4 py-2 text-sm font-medium rounded-xl border border-gray-200 dark:border-neutral-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-all">
                  Cancel
                </button>
                <button
                  onClick={async () => {
                    if (!editStudioForm.name.trim()) return;
                    setActionLoading("edit-studio");
                    try {
                      await fetch("/api/admin?action=update-studio", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                      ...
                      await fetchData("studios");
                    } finally { setActionLoading(""); }
                  }}
                  disabled={actionLoading === "edit-studio" || !editStudioForm.name.trim()}
                  className="px-4 py-2 text-sm font-semibold rounded-xl bg-rose-500 text-white hover:bg-rose-600 disabled:opacity-40 transition-all"
                >
                  {actionLoading === "edit-studio" ? "Saving..." : "Save Changes"}
                </button>
              </div>
            </div>
          </div>
