  ...
  const handleReject = async (artistId: string) => {
    ...
    try {
      await fetch("/api/admin?action=reject-artist", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ artistId, reason: rejectReason || t('moderation.defaultRejectReason') }),
      });
      setArtists((prev) => prev.filter((a) => a.id !== artistId));
      setRejectId(null);
      setRejectReason("");
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('moderation.title')}</h1>
                <p className="text-sm text-gray-500">{t('moderation.description')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-6">
            ...
            >
              {t('moderation.contentModeration')}
            </button>
          </div>
            ...
            <>
              {loading ? (
                ...
                <div className="text-center py-16 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">{t('moderation.allCaughtUp')}</h3>
                  <p className="text-sm text-gray-500">{t('moderation.noPendingVerifications')}</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {artists.map((artist) => (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 flex items-center gap-1">
                              <Clock className="w-3 h-3" /> {t('moderation.pending')}
                            </span>
                          </div>
            ...
            <>
              {items.length === 0 ? (
                <div className="text-center py-16 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">{t('moderation.allClear')}</h3>
                  <p className="text-sm text-gray-500">{t('moderation.noReportsNeedingReview')}</p>
                </div>
              ) : (
                    ...
                    <div key={r.id} className="p-4 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-900 dark:text-white">
                              {r.type === "community" ? t('moderation.communityApplication') : t('moderation.contactMessage')} &mdash; {r.target}
                            </p>
                            <p className="text-xs text-gray-400">{t('moderation.from', { name: r.from })} &middot; {r.date}</p>
                          </div>
                        </div>
                      </div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">{r.reason}</p>
                        ...
                        <button onClick={() => handleResolve(r.id, r.type)} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-lg bg-green-50 text-green-600 hover:bg-green-100">
                          <CheckCircle className="w-3.5 h-3.5" /> {t('moderation.dismiss')}
                        </button>
                      )}
                    </div>
                  ))}
