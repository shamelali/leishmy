...
async function uploadImage(fileName: string, folder?: string): Promise<string> {
  ...
  const publicId = path.parse(fileName).name; // without extension
  console.log(`Uploading ${fileName} as publicId '${publicId}'...`);
...
(async () => {
  ...
  // Upload all images to root folder (no folder) with publicId without extension
  const uploadPromises = files.map(file => uploadImage(file, undefined));
  const urls = await Promise.all(uploadPromises);
  console.log("All images uploaded to root (publicId without extension).");
